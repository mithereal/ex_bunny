defmodule Bunny.Net.Client.Behaviour do
  @moduledoc """
  ...
  if your prefer a different http client. You'll need to set the `:http_client` configuration in `Bunny.Net.Client`:

      client = %Bunny.Net.Client{http_client: {MyHTTPClient, []}}
      Bunny.Net.get(client, "some/file.txt")
  """

  @doc """
  Executes an HTTP request. Must return either {:ok, map} or {:error, reason}.

  - `body` must be raw file data without any encoding. See [BunnyApi API reference]https://docs.bunny.net/reference/put_-storagezonename-path-filename
